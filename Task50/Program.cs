...
PrintMatrix(matrix);
int countI = GetNumber("Введите номер строки: ");
int countJ = GetNumber("Введите номер столбца: ");

if(countI <= countOfRows && countJ <= countOfColumns)
    Console.Write($"Элемент с индексами [{countI},{countJ}] равен {matrix[countI,countJ]} ");
if(countI > countOfRows && countJ > countOfColumns)
